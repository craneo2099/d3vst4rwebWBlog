<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="utf-8"/>
	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<meta http-equiv="X-UA-Compatible" content="IE=edge"/>
	<!-- descripcion -->
	<meta name="description" content="Conócenos somos la mejor opción para tu empresa o negocio somos especialistas en desarrollo de software software, hardware, tecnología y computación. consúltanos antes de iniciar tru proyecto"/>
	<meta name="author" content="Devstar Novatech">
	<!--[if lt IE 9]>
	<script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js">
	</script>
	<![endif]-->
	<link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Poppins">
	<link rel="shortcut icon" href="Img/logo2.webp">
	<title>Devstar Novatech - Soluciones en Inform&aacute;tica</title>
	<link rel="stylesheet" href="css/bootstrap.css">
	<link rel="stylesheet" href="css/font-awesome.min.css">
	<link rel="stylesheet" href="css/slider.min.css">
	<link rel="stylesheet" href="css/main.css">
	<link rel="stylesheet" type="text/css" href="css/qkconsent.css" />
	<script>
		window.dataLayer = window.dataLayer || [];
		function gtag(){dataLayer.push(arguments);}
		gtag('js', new Date());
  
		gtag('config', 'UA-142838512-1');
	  </script>
</head> 
<body data-spy="scroll" data-target="#navbarSupportedContent">
<header id="inicio" class="box1 header">
	<nav id="barranav" class="navbar navbar-expand-lg fixed-top">
	  <img src="Img/logoB.webp" alt="logoBlanco" class="logoBlancoMenu mx-1"/>
	  <!-- <img src="Img/logoBN.webp" alt="logoBlanco" class="logoBlancoMenu mx-1"> -->
	  <a class="navbar-brand text-white barraLogo" href="#">DEVSTAR NOVATECH</a>
	  <!-- <h1 class="navbar-brand text-white barraLogo" href="#">DEVSTAR NOVATECH</h1> -->
	  <button class="navbar-toggler togglerA" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
	    <i class="fa fa-bars"></i>
	  </button>

	  <div class="collapse navbar-collapse" id="navbarSupportedContent">
	    <ul class="navbar-nav ml-auto nav-pills">
	      <li class="nav-item active togglerItem">
	        <a class="nav-link barra  barraLogo collapseJQ" href="#inicio">Inicio</a>
	      </li>
	      <li class="nav-item togglerItem">
	        <a class="nav-link barra collapseJQ" href="#soluciones">Soluciones</a>
	      </li>
	      <li class="nav-item togglerItem">
	        <a class="nav-link barra collapseJQ" href="#servicios">Servicios</a>
	      </li>     
	      <li class="nav-item togglerItem">
	        <a class="nav-link barra collapseJQ" href="#contacto">Contacto</a>
	      </li>
	      <li class="nav-item togglerItem">
	        <a class="nav-link barra collapseJQ" href="blog.html">Blog</a>
	      </li>
	      <li class="nav-item dropdown togglerItem">
	        <a class="nav-link dropdown-toggle barra" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
	          M&aacute;s
	        </a>
	        <div class="dropdown-menu togglerItem" aria-labelledby="navbarDropdown">
	          <a class="dropdown-item dropbar collapseJQ" href="#sobreNos">Sobre Nosotros</a>      
	          <div class="dropdown-divider"></div>
	          <a class="dropdown-item dropbar collapseJQ" href="#finalSec">Acerca de</a>
	        </div>
	      </li>      
	    </ul>
	    <form class="form-inline d-flex justify-content-center my-2">
	    	<input class="form-control mr-sm-2 w-75" type="search" placeholder="Buscar" aria-label="Search">
	    	<button class="btn btn-success my-2 my-sm-0 ml-1" type="submit">
	    		<i class="fa fa-search" aria-hidden="true"></i>
	      	</button>
	    </form>	    
	  </div>
	</nav>
	<div id="devstarTitle" class="container h-100">
		<div class="row justify-content-center align-items-center h-100 relativo" >
			<!-- easter egg -->
			<div class="fondoTitulo rounded col-md-6 col-lg-5 col-xl-4 absoluto">
				<img src="Img/Jomi.jpg" alt="jomi.jpg" class="img-fluid" style="display: none;">
			</div>
			<!-- visible mas 1024px -->
			<div class="fondoTitulo rounded col-md-6 col-lg-5 col-xl-4 absoluto noMostrarMenor576 noMostrar576768 noMostrar7691024">
				<!-- <h2 class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</h2> -->
				<h1 class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</h1>
			</div>
			<!-- visible entre 768px y 1024px -->
			<div class="fondoTitulo rounded col-md-6 col-lg-5 col-xl-4 noMostrarMenor576 noMostrar576768 noMostrarMas1024">
				<!-- <h2 class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</h2> -->
				<p class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</p>
			</div>	
			<!-- visible entre 576px y 768px -->
			<div class="fondoTitulo rounded col-md-6 col-lg-5 col-xl-4 noMostrarMenor576 noMostrar7691024 noMostrarMas1024">
				<!-- <h2 class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</h2> -->
				<p class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</p>
			</div>	
			<!-- visible menor 576px -->
			<div class="fondoTitulo rounded col-md-6 col-lg-5 col-xl-4 noMostrarMas576">
				<!-- <h2 class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</h2> -->
				<p class="titulo">SOLUCIONES EN INFORM&Aacute;TICA</p>
			</div>	
		</div>
<!-- 		<div class="whats ">
			<img src="Img/whats1.webp" alt="" class="imgWhats">
		</div> -->
		<a href="https://api.whatsapp.com/send?phone=525563262446&text=Hola%21%20Quisiera%20m%C3%A1s%20informaci%C3%B3n." class="float1" target="_blank">
		<i class="fa fa-whatsapp my-float"></i>
		</a>
		<!-- scroll down btn-->
		<a href="#soluciones">
      		<div class="scroll-down"></div>
    	</a>
	</div>	
</header>
<!-- section main -->
<section id="soluciones" class="py-4 soluciones">
	<div class="container my-5">
		<h2 class="titulo2 animTop">SOLUCIONES</h2>
		<div class="row justify-content-center pt-4 ">
			<div class="col-md-3 ">
				<div class="ocultarGrande card solucionesInfo py-2">
					<img src="Img/img_trans.gif" alt="aplicaciones.webp" class="card-img-top mx-auto d-block align-self-center aplicaciones">
					<div class="card-body border-white border-bottom mt-md-4">
						<div class="d-flex justify-content-center">
							<h3 class="titulo5">APLICACIONES</h3>			
						</div>
						<p class="card-text text-justify">Nosotros plasmamos sus ideas con la mejor tecnolog&iacute;a en dispositivo.</p>
						<div class="d-flex justify-content-center ocultarGrande">
    						<a href="#aplicaciones" class="shadow btn btnSoluciones">Ver M&aacute;s</a>
						</div>	
					</div>
				</div>
				<div class="ocultarChico card solucionesInfo pb-2 animIzq">	
					<img src="Img/img_trans.gif" alt="aplicaciones.webp" class="card-img-top mx-auto d-block align-self-center aplicaciones">
					<div class="card-body border-white border-top mt-4">
						<div class="d-flex justify-content-center">
							<h3 class="titulo5">APLICACIONES</h3>							
						</div>
						<p class="card-text text-justify">Plasmamos sus ideas con la mejor tecnolog&iacute;a en cualquier dispositivo.</p>	
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="ocultarGrande card solucionesInfo py-2">
					<img src="Img/img_trans.gif" alt="mantenimiento.webp" class="card-img-top mx-auto d-block align-self-center mantenimiento">
					<div class="card-body border-white border-bottom borderChicoNone mt-md-4">
						<div class="d-flex justify-content-center">
							<h3 class="titulo5">MANTENIMIENTO</h3>							
						</div>
						<p class="card-text text-justify">Nos adaptamos a sus necesidades, servicio 24 horas 365 d&iacute;as al a&ntilde;o.</p>
						<div class="ocultarGrande d-flex justify-content-center">
    						<a href="#mantenimiento" class="shadow btn btnSoluciones">Ver M&aacute;s</a>
						</div>
					</div>
				</div>
				<div class="ocultarChico card solucionesInfo pb-2 animTop">
					<img src="Img/img_trans.gif" alt="mantenimiento.webp" class="card-img-top mx-auto d-block align-self-center mantenimiento">
					<div class="card-body border-white border-top mt-4">
						<div class="d-flex justify-content-center">
							<h3 class="titulo5">MANTENIMIENTO</h3>							
						</div>
						<p class="card-text text-justify">Nos adaptamos a sus necesidades, servicio 24 horas 365 d&iacute;as al a&ntilde;o.</p>
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="ocultarGrande card solucionesInfo justify-content-center py-2">
					<img src="Img/img_trans.gif" alt="consultoria.webp" class="card-img-top mx-auto d-block align-self-center consultoria">
					<div class="card-body mt-md-4">
						<div class="d-flex justify-content-center">
							<h3 class="titulo5">CONSULTOR&Iacute;A</h3>
						</div>
						<p class="card-text text-justify">El impulso que necesita para enfrentar los desaf&iacute;os tecnol&oacute;gicos de su empresa o negocio.</p>
						<div class="d-flex justify-content-center">
    						<a href="#consultoria" class="shadow btn btnSoluciones">Ver M&aacute;s</a>
						</div>
					</div>
				</div>
				<div class="ocultarChico card solucionesInfo justify-content-center pb-2 animDer">
					<img src="Img/img_trans.gif" alt="consultoria.webp" class="card-img-top mx-auto d-block align-self-center consultoria">
					<div class="card-body border-white border-top mt-4">
						<div class="d-flex justify-content-center">
							<h3 class="titulo5">CONSULTOR&Iacute;A</h3>
						</div>
						<p class="card-text text-justify">El impulso que necesita para enfrentar los desaf&iacute;os tecnol&oacute;gicos de su empresa o negocio.</p>						
					</div>
				</div>
			</div>
		</div>
		<div class="row justify-content-center ocultarChico">
			<div class="col-md-3 animIzq">
				<div class="d-flex justify-content-center">
    				<a href="#aplicaciones" class="shadow btn btnSoluciones">Ver M&aacute;s</a>
				</div>
			</div>
			<div class="col-md-3 animTop">
				<div class="d-flex justify-content-center">
    				<a href="#mantenimiento" class="shadow btn btnSoluciones">Ver M&aacute;s</a>
				</div>
			</div>
			<div class="col-md-3 animDer">
				<div class="d-flex justify-content-center">
    				<a href="#consultoria" class="shadow btn btnSoluciones">Ver M&aacute;s</a>
				</div>
			</div>
		</div>
	</div>
</section>
<section class="py-2 py-md-4 solucionesDos">
	<div class="container">		
		<div class="row justify-content-center py-4 py-md-0 py-lg-4">			
			<div class="col-md-3 col-lg-3 my-auto animIzq">				
				 <div class="shadow border border-white rounded-right labelSolDos">
				 	<p class="p-1 m-0 font-weight-bold">1. Soluciones de TI gestionadas</p>
				 </div>
				<div class="shadow border border-white rounded-right labelSolDos mt-3">
				 	<p class="p-1 m-0 font-weight-bold">2. Desarrollos Web</p>
				 </div>
				 <div class="shadow border border-white rounded-right labelSolDos mt-3">
				 	<p class="p-1 m-0 font-weight-bold">3. Hosting</p>
				 </div>				
			</div>

			<div class="col-md-4 col-lg-3 py-3 py-sm-0 my-md-auto animIzq">
				<div class="d-flex justify-content-center">
    				<img src="Img/devAnt.webp" alt="logoDevAnt" class="logoDevAnt img-fluid">
				</div>
			</div>

			<div class="col-md-4 col-lg-5 my-auto animIzq">
				<div class="">
					<h3>Ayud&aacute;ndole a encontrar las soluciones de TI adecuadas.</h3>
					<p class="text-justify">Nuestro modelo “Flexible” de servicio se adapta a sus requerimientos ofreciendo la mejor relaci&oacute;n de calidad y precio, haciendo hincapi&eacute; en la competitividad t&eacute;cnica y econ&oacute;mica.</p>				
				</div>		
			</div>
		</div>

	</div>
</section>
<section id="servicios" class="box4">
	<div class="container h-100">
		<div class="row justify-content-center align-items-center h-100">
			<h2 class="tituloSol2">SERVICIOS</h2>
		</div>
	</div>
</section>
<section id="aplicaciones" class="pt-4 solucionesTres">
	<div class="container ">
		<h3 class="titulo2 animTop">ALIADO DE "TI" CON VISI&Oacute;N COMPARTIDA</h3>
		<div class="row justify-content-center pt-4 pb-2 mt-2">			
			<div class="col-md-3 my-auto animIzq">
				<img src="Img/img_trans.gif" alt="quien" class="mx-auto d-block align-self-center appWeb">				 
			</div>
			<div class="col-md-7 animDer">
				<h4 class="titulo4">APLICACIONES WEB</h4>
				<p class="text-justify">					
				Una aplicaci&oacute;n web es una pagina de internet interactiva que te puede facilitar el control de tu empresa donde te encuentres. Utilizando “la nube” puedes tener una aplicaci&oacute;n web, incluso sin necesitar tener una computadora.
				</p>
			</div>		
		</div>
		<div class="row justify-content-center py-2">
			<div class="col-md-3 my-auto animIzq">
				<img src="Img/img_trans.gif" alt="quien" class="mx-auto d-block align-self-center appMov ">				 
			</div>
			<div class="col-md-7 animDer">
				<h4 class="titulo4">APLICACIONES M&Oacute;VILES</h4>
				<p class="text-justify">					
				Todos hemos utilizado una aplicaci&oacute;n M&oacute;vil, ya sea en tu tel&eacute;fono, tablet o wearable, pero no todos saben como hacerlas y hacerlas bien. No importa si es Android, IOS o lo que venga, simplemente ponte en contacto con nosotros y encontraremos una soluci&oacute;n. 
				</p>				
			</div>			
		</div>
		<div class="row justify-content-center py-2 mb-2">
			<div class="col-md-3 my-auto animIzq">
				<img src="Img/img_trans.gif" alt="quien" class="mx-auto d-block align-self-center webSite ">				 
			</div>
			<div class="col-md-7 animDer">
				<h4 class="titulo4">SITIOS WEB</h4>
				<p class="text-justify">					
				Es tu carta de presentaci&oacute;n hacia el mundo, haz que sea &uacute;nica y haz que refleje la verdadera identidad de tu marca 
				</p>				
			</div>			
		</div>
		<div class="row justify-content-center py-2 mt-2">
			<div id="mantenimiento" class="col-md-5 justify-content-center animIzq">
				<h3 class="titulo3 pb-2 text-center" >MANTENIMIENTO</h3>
				<ul class="">
					<li>Desarrollo, mantenimiento y distribuci&oacute;n de software, Apps y sitis web.</li>
					<li>Arrendamiento, Mantenimiento y venta de equipo de c&oacute;mputo y redes.</li>					
					<li>Administraci&oacute;n de bases de datos.</li>					
					<li>An&aacute;lisis y optimizaci&oacute;n de procesos de TI.</li>
					<li>Desarrollo y mantenimiento de Sistemas biom&eacute;tricos.</li>					
				</ul>
			</div>
			<div class="col-md-5 animIzq">
				<img src="Img/mante.jpg" alt="mante.jpg" class="rounded mx-auto d-block align-self-center img-fluid">		
			</div>
		</div>
		<div class="row justify-content-center py-2 mt-2">
			<div id="consultoria" class="col-md-5 animDer">
				<h3 class="titulo3 pb-2 text-center">CONSULTOR&Iacute;A</h3>
				<ul>				
					<li>Adopci&oacute;n Tecnol&oacute;gica.</li>
					<li>Consultor&iacute;a Estrat&eacute;gica.</li>
					<li>Cloud computing.</li>
					<li>Gesti&oacute;n del Cambio.</li>					
					<li>Dise&ntilde;o de sistemas a la medida.</li>
					<li>Automatizaci&oacute;n de procesos.</li>
					<li>Planificaci&oacute;n de recursos empresariales (ERP)</li>
					<li>Gesti&oacute;n de clientes (CRM)</li>
				</ul>
			</div>
			<div class="col-md-5 animDer">
				<img src="Img/consul.jpg" alt="consul.jpg" class="rounded mx-auto d-block align-self-center img-fluid">		
			</div>						
		</div>
<!-- 		<div class="row justify-content-center py-2">			
			<div class="col-md-5">
				<h3 class="titulo3 pb-2 text-center">PRODUCTOS</h3>
				<ul>
					<li>Cloud computing.</li>
					<li>Digitalizaci&oacute;n, preservaci&oacute;n y administraci&oacute;n de documentos y material hist&oacute;rico.</li>
					<li>Ingenier&iacute;a inversa y miner&iacute;a de datos.</li>
					<li>Monitoreo remoto de sistemas.</li>					
					<li>Implementaciones de Realidad virtual, visi&oacute;n artificial, Inteligencia artificial, reconocimiento de patrones.</li>
					<li>Servicios de streaming de contenido multimedia en la red.</li>

					<li>Sistemas inteligentes.</li>
				</ul>
			</div>						
		</div> -->
		<div class="row justify-content-center py-4 animIzq">
			<div class="col-md-10 my-auto noMostrarMenor768">
				<img src="Img/fondoSol2.webp" alt="fondoSol2" class="rounded mx-auto d-block align-self-center img-fluid">		
			</div>			
		</div>
		<div class="row justify-content-center py-4 animBo2">			
			<div class="col-md-5 d-flex justify-content-center pb-2">
				<a href="#contacto" class="btn btn-outline-primary btn-lg shadow" >CONTACTO</a>
			</div>						
		</div>
	</div>
</section>
<section id="sobreNos" class="box2">
	<div class="container h-100">
		<div class="row justify-content-center align-items-center h-100">
			<h2 class="tituloSobre">SOBRE NOSOTROS</h2>
		</div>
	</div>
</section>
<section class="pt-4 sobreNostros">
	<div class="container ">
		<h3 class="titulo2 animTop">MEET DEVSTAR</h3>
		<div class="row justify-content-center py-4 animIzq">			
			<div class="col-md-3 my-auto">
				<img src="Img/img_trans.gif" alt="quien" class="mx-auto d-block align-self-center quienSomos ">				 
			</div>
			<div class="col-md-7">
				<h4 class="titulo3">QUI&Eacute;NES SOMOS</h4>
				<p class="text-justify pt-4">					
				Somos una empresa mexicana, que surge de la necesidad de mejorar los procesos por medio de tecnolog&iacute;a disruptiva, con el fin aumentar el desempe&ntilde;o, rendimiento y producci&oacute;n de las tareas diarias. 
				</p>
				<p class="text-justify">
				Nuestro equipo tiene m&aacute;s de 10 a&ntilde;os de experiencia tanto en gobierno como en empresas particulares, dando como resultado una compa&ntilde;&iacute;a focalizada en el desarrollo de soluciones de tecnolog&iacute;as, de metodolog&iacute;as y de alcance para satisfacer las necesidades en TI de cualquier cliente.
				</p>
			</div>			
		</div>
		<div class="row justify-content-center py-4 animIzq">			
			<div class="col-md-3 my-auto">
				<img src="Img/img_trans.gif" alt="vision" class="mx-auto d-block align-self-center vision ">				 
			</div>
			<div class="col-md-7">
				<h4 class="titulo3">VISI&Oacute;N</h4>
				<p class="text-justify pt-4">					
				Ser reconocidos por nuestra contribuci&oacute;n a la sociedad facilitando el acceso a herramientas tecnol&oacute;gicas que aporten a la mejora y crecimiento de nuestros clientes.
				</p>				
			</div>
		</div>
		<div class="row justify-content-center py-4 animIzq">			
			<div class="col-md-3 my-auto">
				<img src="Img/img_trans.gif" alt="quien" class="mx-auto d-block align-self-center mision">				 
			</div>
			<div class="col-md-7">
				<h4 class="titulo3">MISI&Oacute;N</h4>
				<p class="text-justify pt-4">					
				Se basa en ofrecer soluciones de tecnolog&iacute;a que cumplan con las siguientes caracter&iacute;sticas:
				</p>
				<ul>
					<li>Robustas, estables y confiables.</li>
					<li>F&aacute;ciles de implementar.</li>
					<li>F&aacute;ciles de administrar.</li>
					<li>Intuitivas para los usuarios finales.</li>	
				</ul>
			</div>
		</div>
		<div class="row justify-content-center py-4 animIzq">			
			<div class="col-md-3 my-auto">
				<img src="Img/img_trans.gif" alt="quien" class="mx-auto d-block align-self-center tecnologia">				 
			</div>
			<div class="col-md-7">
				<h4 class="titulo3">TECNOLOG&Iacute;A</h4>
				<p class="text-justify pt-4">					
				Contamos con gran variedad de opciones en TI para resolver sus necesidades, nuestras soluciones se acomodan a todos los presupuestos, con nuestra asesor&iacute;a podr&aacute; definir cu&aacute;l es la mejor opci&oacute;n, en que &aacute;rea y secci&oacute;n es m&aacute;s viable para implementar una soluci&oacute;n para usted.
				</p>
			</div>				
		</div>

	</div>
</section>

<section id="contacto" class="box3">
	<div class="container h-100">
		<div class="row justify-content-center align-items-center h-100">
			<h2 class="tituloc ">CONTACTO</h2>
		</div>
	</div>
</section>
<section class="pt-4">
	<div class="container contacto">		
		<h3 class="titulo2 animTop">CUBRIMOS SUS NECESIDADES</h3>
		<div class="row justify-content-center py-4">			
			<div class="col-md-4 pb-3 pb-sm-1 animIzq">				
				<aside class="">
					<h4 class="titulo4 text-uppercase">D&iacute;ganos sus necesidades y nosotros le proponemos una soluci&oacute;n</h4>
				</aside>
			</div>
			<form id="contactForm" action="" class="col-md-7 animDer">
				<div class="form-group">
					<label for="email" class="control-label col-md-2 sr-only">Email*:</label>
					<div class="col-md-10 mx-auto">
						<input id="email" type="email" class="form-control contacto" placeholder="Email*" required >
					</div>				
				</div>				
				<div class="form-group">
					<label for="phone" class="control-label col-md-2 sr-only">Tel&eacute;fono::</label>
					<div class="col-md-10 mx-auto">
						<input id="phone" type="text" class="form-control contacto" placeholder="Tel&eacute;fono" required>
					</div>				
				</div>
				<div class="form-group">
					<label for="message" class="control-label col-md-2 sr-only ">Mensaje:</label>
					<div class="col-md-10 mx-auto">
						<textarea name="message" id="message"  placeholder="Mensaje" rows="5" class="form-control contacto" required></textarea>
					</div>
				</div>
				<div class="form-group">
				<?php require_once "lib/utilities.php";
				$captcha=getCaptcha();
				?>
					<label for="captcha" class="control-label col-md-2 sr-only">Captcha:</label>
					<div class="col-md-10 mx-auto">
						<div id="captcha" class="row">
							<div class="col-md-7 pb-1">
								<!-- sustituir por captcha -->
								<img src="<?=$captcha->result->captcha?>"  alt="captcha" class="">	
							</div>
							<div class="col-md-5 ">
								<input id="captcha" type="text" class="form-control contacto" placeholder="Escribe el Texto" required>
								<input id="tokenKcha" type="hidden" value="<?=$captcha->result->token?>" >
							</div>							
						</div>
					</div>
				</div>
				<div class="form-group ">
					<div id="success"></div>
					<div class="col-3 col-sm-4 col-lg-3 mx-auto mx-sm-1 mx-lg-0 ml-sm-auto ml-lg-auto">
						
						<button type="submit" id="sendMessageButton" class="btn contacto contactoBtn">Enviar</button>		
					</div>
				</div>
			</form>
		</div>		
	</div>
</section>
<!-- final -->
<section id="finalSec" class="py-1 py-md-4 finalSeccion">
	<div class="container">		
		<div class="row justify-content-center">
			<div class="col-md-3">
				<div class="finalInfo finalInfo1st">					
					<div class="row justify-content-center pt-2">
						<div class="col-4 col-md-4 py-2 py-md-0 mt-lg-2 my-auto">
							<img src="Img/logoB.webp" alt="logoBlanco" class="logoFinBlanco">
						</div>
						<div class="col-7 col-md-8 mt-md-2 px-0 my-auto my-lg-0">
							<h2 class="titulo4F pt-2">DEVSTAR NOVATECH</h2>
						</div>
					</div>
				</div>
			</div>	
			<div class="col-md-3 ">
				<div class="finalInfo pt-2">					
					<h3 class="titulo5">ENLACES</h3>
					<ul class="listFinal">
						<li class="px-1"><a class="linkFinal" href="#inicio">Inicio</a></li>
						<li class="px-1"><a class="linkFinal" href="#soluciones">Soluciones</a></li>
						<li class="px-1"><a class="linkFinal" href="#servicios">Servicios</a></li>
						<li class="px-1"><a class="linkFinal" href="#contacto">Contacto</a></li>
						<li class="px-1"><a class="linkFinal" href="#sobreNos">Sobre Nosotros</a></li>						
					</ul>
				</div>
			</div>				
			<div class="col-md-3">
				<div class="finalInfo pt-2">					
					<h3 class="titulo5">CONTACTO</h3>
					<div class="row justify-content-center">
						<div class="col-1 col-sm-1 my-auto">
							<i class="fa fa-envelope-o"></i>				
						</div>
						<div class="col-7 col-sm-10 plh">
							<a href="mailto:contacto@devstar-novatech.com" class="linkFinal" target="_top">contacto@devstar-novatech.com</a>			
						</div>
						<div class="clearfix"></div>
					</div>
					<div class="row justify-content-center">
						<div class="col-1 col-sm-1">
							<i class="fa fa-phone"></i>
						</div>
						<div class="col-7 col-sm-10 plh">
							<p class="mb-0"><a href="tel:+(52) 5563262446" style="color: white;">5563262446</a></p>
						</div>
					</div>
					<div class="row justify-content-center">
						<div class="col-1 col-sm-1">
							<i class="fa fa-user-circle-o"></i>
						</div>
						<div class="col-7 col-sm-10 plh">
							<p class="mb-0">Lunes a Viernes, 9 - 18 Hrs.</p>
						</div>
					</div>
					<div class="row justify-content-center">
						<div class="col-1 col-sm-1">
							<i class="fa fa-map-marker"></i>
						</div>
						<div class="col-7 col-sm-10 plh">
							<p>CDMX, M&eacute;xico</p>
						</div>
					</div>
				</div>
			</div>
			<div class="col-md-3 ">
				<div class="finalInfo pt-2">					
					<h3 class="titulo5">REDES SOCIALES</h3>
					<div class="portCh">
						<a href="#" class="btn btn-white">
							<i class="fa fa-twitter iconFinalRedes"></i>
						</a>
						<a href="" class="btn btn-white">
							<i class="fa fa-facebook iconFinalRedes"></i>
						</a>
						<a href="" class="btn btn-white">
							<i class="fa fa-google-plus iconFinalRedes"></i>
						</a>
					</div>
				</div>
			</div>
			
		</div>		
	</div>
</section>
<footer class="d-flex justify-content-center py-1" >
	<small><strong>Devstar Novatech 2021 </strong><i class="fa fa-copyright" aria-hidden="true"></i></small>
</footer>

<script src="js/jquery.min.js" ></script>
<script src="js/popper.min.js" ></script>
<script src="js/bootstrap.min.js" ></script>
<script src="js/jqBootstrapValidation.js" ></script>
<script src="js/scrollreveal.min.js" ></script>
<script src="js/propio.js" ></script>
<script src="js/qkconsent.js" data-cfasync="false"></script>
<script src="js/devstar.js" ></script>
<script>
window.addEventListener('load', function(){
  window.cookieconsent.initialise({
   revokeBtn: "<div class='cc-revoke'></div>",
   type: "opt-in",
   theme: "classic",
   palette: {
       popup: {
           background: "rgb(28 44 23 / 90%)",
           text: "#fff"
        },
       button: {
           background: "#CAD160",
           text: "#000"
        }
    },
   content: {
       message: "Utilizamos cookies para mejorar la experiencia de los usuarios y para analizar "+
       "la concurrencia al sitio web. Por estas razones, es posible que compartamos los datos que ha"+
       " proporcionado con nuestros socios de analíticas web. Al hacer clic en \"Acepto cookies\","+
       " usted autoriza el almacenamiento de todas las tecnologías descriptas en nuestra Política de"+
       " cookies en su dispositivo.",
       link: "politicas",
       allow: "Acepto cookies",
       deny: "Rechazar",
       href: "mrvcookie.com/cookies"
    },
    onInitialise: function(status) {
      if(status == cookieconsent.status.allow) myScripts();
    },
    onStatusChange: function(status) {
      if (this.hasConsented()) myScripts();
    }
  })
});

function myScripts() {

   // Paste here your scripts that use cookies requiring consent. See examples below

   // Google Analytics, you need to change 'UA-00000000-1' to your ID
      (function(wdw,doc,tag,url,ja,a,m){
		  a=doc.createElement(tag),m=doc.getElementsByTagName(tag)[0];
		  a.async=1;
		  a.src=url;
		  m.parentNode.insertBefore(a,m)
      })(window,document,'script','https://www.googletagmanager.com/gtag/js?id=UA-142838512-1','gtag');
}
</script>
</body>
</html>
